...
class TestTopicService(unittest.TestCase):
    ...
    def test_get_by_id_success(self):
        ...
        self.db.query.return_value.filter.return_value.first.return_value = Topic(id=self.topic_id)
    ...
    def test_create_success(self):
        ...
        self.db.query.return_value.filter.return_value.first.return_value = None  # Assume category exists

        # Mock Topic creation
        new_topic = Topic(id=self.topic_id, title="New Topic", category_id=self.category_id, author_id=self.user_id)
    ...
    def test_update_success(self):
        # Mock existing topic
        existing_topic = Topic(id=self.topic_id, author_id=self.user_id, title="Old Title")
        self.db.query.return_value.filter.return_value.first.return_value = existing_topic
        ...
        self.db.query.return_value.filter.return_value.first.return_value = Reply(id=uuid4())  # Assume valid reply
        self.db.query.return_value.filter.return_value.first.return_value = Topic(id=self.category_id)  # Assume valid category
    ...
    def test_update_topic_not_found(self):
        ...
        # Expect an HTTPException to be raised
        with self.assertRaises(HTTPException) as context:
            update(User(id=self.user_id), self.topic_id, TopicUpdate(title="Updated Title", category_id=self.category_id), self.db)
    ...
    def test_select_best_reply_success(self):
        ...
        reply_id = uuid4()
        self.db.query.return_value.filter.return_value.first.return_value = topic
        self.db.query.return_value.filter.return_value.first.return_value = Reply(id=reply_id)

        # Call the select_best_reply function
        updated_topic = select_best_reply(User(id=self.user_id), self.topic_id, reply_id, self.db)
