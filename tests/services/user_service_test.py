...
class UserService_Should(unittest.TestCase):
    ...
    def test_create_returnsCreatedUser(
        self, mock_hash_password, mock_ensure_unique_username_and_email
    ) -> None:
        # Arrange
        ...
        created_user = user_service.create(
            user_data=user_creation_data, db=self.mock_db
        )
    ...
    def test_getPrivilegedUsers_returnsCorrect_whenCategoryIsFound(
        self, mock_get_category_by_id
    ) -> None:
        # Arrange
        category_id = uuid4()
        user1_category_permission = UserCategoryPermission(
            user=self.user, category_id=category_id, access_level=AccessLevel.READ
        )
        user2_category_permission = UserCategoryPermission(
            user=self.user2, category_id=category_id, access_level=AccessLevel.WRITE
        )
    ...
    @patch("forum_system_api.services.category_service.get_by_id")
    def test_getPrivilegedUsers_raises404_whenCategoryIsNotFound(
        self, mock_get_category_by_id
    ) -> None:
        # Arrange
        category_id = uuid4()
    ...
    @patch("forum_system_api.services.category_service.get_by_id")
    def test_getPrivilegedUsers_raises400_whenCategoryIsNotPrivate(
        self, mock_get_category_by_id
    ) -> None:
        # Arrange
        category_id = uuid4()
        ...
        self.assertEqual(
            status.HTTP_400_BAD_REQUEST, exception_ctx.exception.status_code
        )
        self.assertIn("Category is not private", str(exception_ctx.exception))

    @patch("forum_system_api.services.user_service.get_by_id")
    def test_getUserPermissions_returnsCorrect_whenUserIsFound(
        self, mock_get_by_id
    ) -> None:
        # Arrange
        category_id = uuid4()
    ...
    @patch("forum_system_api.services.user_service.get_by_id")
    def test_getUserPermissions_raises404_whenUserIsNotFound(
        self, mock_get_by_id
    ) -> None:
        # Arrange
    ...
    def test_revokeAccess_returnsTrue_whenPermissionIsFound(
        self, mock_get_user_category_permission
    ) -> None:
        # Arrange
        user_category_permission = Mock()
        mock_get_user_category_permission.return_value = (
            self.user,
            None,
    ...
    @patch("forum_system_api.services.user_service.get_user_category_permission")
    def test_revokeAccess_raises404_whenPermissionIsNotFound(
        self, mock_get_user_category_permission
    ) -> None:
        # Arrange
    ...
    def test_getUserCategoryPermission_returnsCorrect_whenPermissionIsFound(
        self, mock_get_by_id, mock_get_category_by_id
    ) -> None:
        # Arrange
        ...
        result = user_service.get_user_category_permission(
            user_id=self.user.id, category_id=category.id, db=self.mock_db
        )
    ...
    def test_getUserCategoryPermission_raises404_whenUserIsNotFound(
        ...
        with self.assertRaises(HTTPException) as exception_ctx:
            user_service.get_user_category_permission(
                user_id=self.user.id, category_id=uuid4(), db=self.mock_db
            )
    ...
    def test_ensureUniqueUsernameAndEmail_raises409_whenUsernameAlreadyExists(
        ...
        # Act & Assert
        with self.assertRaises(HTTPException) as exception_ctx:
            user_service.ensure_unique_username_and_email(
                self.user.username, self.user.email, self.mock_db
            )
    ...
    def test_ensureUniqueUsernameAndEmail_raises409_whenEmailAlreadyExists(
        self, mock_get_by_email, mock_get_by_username
    ) -> None:
        # Arrange
