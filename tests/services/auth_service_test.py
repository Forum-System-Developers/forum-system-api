...
class AuthService_Should(unittest.TestCase):
    ...
    def test_createToken_raises500_whenTokenCreationFails(
        self, mock_jwt_encode
    ) -> None:
        # Arrange
        ...
        self.assertEqual(
            status.HTTP_500_INTERNAL_SERVER_ERROR, ctx.exception.status_code
        )
    ...
    def test_createAccessAndRefreshTokens_returnsTokens(
        ...
        token_response = auth_service.create_access_and_refresh_tokens(
            user=self.user, db=self.mock_db
        )
    ...
    @patch("forum_system_api.services.auth_service.create_access_token")
    def test_refreshAccessToken_returnsToken(
        self, mock_create_access_token, mock_verify_token
    ) -> None:
        # Arrange
        ...
        token = auth_service.refresh_access_token(
            refresh_token=self.mock_refresh_token, db=self.mock_db
        )
    ...
    def test_verifyToken_returnsPayload(self, mock_get_by_id, mock_jwt_decode) -> None:
        ...
        result = auth_service.verify_token(
            token=self.mock_access_token, db=self.mock_db
        )
    ...
    @patch("forum_system_api.services.auth_service.jwt.decode")
    @patch("forum_system_api.services.auth_service.user_service.get_by_id")
    def test_verifyToken_raises401_whenUserNotFound(
        self, mock_get_by_id, mock_jwt_decode
    ) -> None:
        # Arrange
        mock_jwt_decode.return_value = self.payload
    ...
    @patch("forum_system_api.services.auth_service.jwt.decode")
    @patch("forum_system_api.services.auth_service.user_service.get_by_id")
    def test_verifyToken_raises401_whenTokenVersionMismatch(
        self, mock_get_by_id, mock_jwt_decode
    ) -> None:
        # Arrange
    ...
    def test_updateTokenVersion_returnsTokenVersion(self, mock_uuid4) -> None:
        ...
        # Act
        token_version = auth_service.update_token_version(
            user_id=self.user.id, db=self.mock_db
        )
    ...
    def test_updateTokenVersion_raises404_whenUserIsNotFound(
        self, mock_get_user_by_id
    ) -> None:
        mock_get_user_by_id.return_value = None
        # Act & Assert
        with self.assertRaises(HTTPException) as ctx:
            auth_service.update_token_version(user_id=uuid4(), db=self.mock_db)
    ...
    def test_authenticateUser_returnsUser(
        self, mock_get_by_username, mock_verify_password
    ) -> None:
        # Arrange
        ...
        user = auth_service.authenticate_user(
            username=self.user.username, password=VALID_PASSWORD, db=self.mock_db
        )
    ...
    @patch("forum_system_api.services.user_service.get_by_username")
    def test_authenticateUser_raises401_whenUserNotFound(
        self, mock_get_by_username
    ) -> None:
        # Arrange
    ...
    @patch("forum_system_api.services.auth_service.verify_token")
    def test_authenticateWebsocketUser_returnsNone_whenTokenVerificationFails(
    ...
    def test_createTokenData_returnsPayload(
        self, mock_update_token_version, mock_is_admin
    ) -> None:
        # Arrange
        ...
        # Assert
        mock_update_token_version.assert_called_once_with(
            user_id=self.user.id, db=self.mock_db
        )
    ...
    @patch("forum_system_api.services.user_service.get_by_id")
    @patch("forum_system_api.services.auth_service.verify_token")
    def test_getCurrentUser_returnsUser(
        self, mock_verify_token, mock_get_user_by_id
    ) -> None:
        # Arrange
        mock_verify_token.return_value = {"sub": str(self.user.id)}
    ...
    @patch("forum_system_api.services.auth_service.verify_token")
    def test_getCurrentUser_raises401_whenTokenIsInvalid(
        self, mock_verify_token
    ) -> None:
        # Arrange
        mock_verify_token.side_effect = HTTPException(
