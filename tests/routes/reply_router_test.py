...
class ReplyRouterShould(unittest.TestCase):
    ...
    def test_get_by_id_returns200_onSuccess(self):
        with (
            patch(
            'forum_system_api.services.reply_service.get_by_id', return_value=self.reply),
            patch(
    ...
    def test_get_by_id_returns404_replyNotFound(self):
        with (
            patch(
            'forum_system_api.services.reply_service.get_by_id', side_effect=HTTPException(status_code=404, detail="Reply not found")
            )
        ):
            app.dependency_overrides[get_db] = lambda: self.db
            app.dependency_overrides[get_current_user] = lambda: self.user
    ...
    def test_get_by_id_returns403_noTopicPermissions(self):
        with (
            patch(
            'forum_system_api.services.reply_service.get_by_id', side_effect=HTTPException(status_code=403, detail="Unauthorized")
            )
        ):
            app.dependency_overrides[get_db] = lambda: self.db
            app.dependency_overrides[get_current_user] = lambda: self.user
    ...
    def test_create_returns_201_onSuccess(self):
        with (
            ...
            response = self.client.post(f"/api/v1/replies/{self.topic.id}", json=tobj.VALID_REPLY_CREATE)
    ...
    def test_create_returns_403_noPermissions(self):
        with (
            patch(
            'forum_system_api.services.reply_service.create', side_effect=HTTPException(status_code=403, detail="Unauthorized"))
        ):
            app.dependency_overrides[get_db] = lambda: self.db
            app.dependency_overrides[get_current_user] = lambda: self.user
    ...
    def test_update_returns_201_onSuccess(self):
        with (
            patch(
            'forum_system_api.services.reply_service.update', return_value=self.reply),
            patch(
            'forum_system_api.services.reply_service.get_votes', return_value=(1, 0))
        ):
            app.dependency_overrides[get_db] = lambda: self.db
            ...
            response = self.client.put(f"/api/v1/replies/{self.reply.id}", json=tobj.VALID_REPLY_CREATE)
    ...
    def test_update_returns_403_noPermissions(self):
        with (
            ...
            response = self.client.post(f"/api/v1/replies/{self.reply.id}", json=tobj.VALID_REPLY_CREATE)
    ...
    def test_update_returns_404_replyNotFound(self):
        with (
            patch(
            'forum_system_api.services.reply_service.create', side_effect=HTTPException(status_code=404, detail="Reply not found"))
        ):
            app.dependency_overrides[get_db] = lambda: self.db
            app.dependency_overrides[get_current_user] = lambda: self.user
    ...
    def test_create_reaction_returns_201_onSuccess(self):
        with (
            patch(
            ...
            response = self.client.patch(f"/api/v1/replies/{self.reply.id}/vote", json=tobj.VALID_REPLY_REACTION_CREATE_TRUE)
    ...
    def test_create_reaction_returns_404_replyNotFound(self):
        with (
            patch(
            'forum_system_api.services.reply_service.vote', side_effect=HTTPException(status_code=404, detail="Reply not found"))
        ):
            app.dependency_overrides[get_db] = lambda: self.db
            app.dependency_overrides[get_current_user] = lambda: self.user
    ...
    def test_create_reaction_returns_405_noPermissions(self):
        with (
            patch(
            'forum_system_api.services.reply_service.vote', side_effect=HTTPException(status_code=405, detail="Unauthorized"))
        ):
            app.dependency_overrides[get_db] = lambda: self.db
            app.dependency_overrides[get_current_user] = lambda: self.user
